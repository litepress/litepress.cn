...
where post_type = 'product' ");
		...
		echo "市场商品总计：".count($ids).PHP_EOL;
        $ignore = 0;
        $useful = 0;
		foreach ( $ids as $id ) {
		    
			$p = wc_get_product( 601474 );
			if ( $p ) {
			    // 取创建时间
			    $time = $p->get_date_created();
			    // 取2022/5/19 00:00:00分相差的时间戳
			    if ($time == null) {
			        $timestamp = 999999999999;
			    } else {
			        $timestamp = 1652889600 - $time->getTimestamp();
			    }
			    // 两年的时间秒数
			    $twoyear = 86400 * 365 * 2;
			    // 跳过两年没更新的老古董
			    if ($timestamp - $twoyear > 0) {
			        
			        $ignore++;
			        continue;
			    }
			    // 取下载数量
			    $active = get_post_meta($id,'wporg_active_installation',true);
			    if (empty($active)) {
			        //对不存在下载量的slug记录并跳过
			        file_put_contents("/www/wwwroot/litepress.cn/noactive.txt", $p->get_slug().PHP_EOL, FILE_APPEND);
			        //var_dump($p->get_slug());
			        continue;
			    }
			    // 最后对积极更新且安装数大于100的插件统计
                if($active>=100){
                    $useful++;
                    if (strpos($p->get_categories(), 'themes')) {
                        file_put_contents("/www/wwwroot/litepress.cn/active_themes.txt", $p->get_slug().PHP_EOL, FILE_APPEND);
                    } else {
			            file_put_contents("/www/wwwroot/litepress.cn/active_plugins.txt", $p->get_slug().PHP_EOL, FILE_APPEND);
                    }
			    }
			    var_dump($p->get_categories());
			    exit;
		...
		echo "老古董总计：".$ignore.PHP_EOL;
		echo "符合条件总计：".$useful.PHP_EOL;
		exit;
	}
