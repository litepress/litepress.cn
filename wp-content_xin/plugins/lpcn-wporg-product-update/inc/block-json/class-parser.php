...
namespace LitePress\Store\WPOrg_Product_Update\Block_JSON;
...
class Parser {
	...
	public static function parse( array $resource ) {
		...
		$content = match ( $type ) {
			'url' => self::extract_content_from_url( $handle ),
			'file' => self::extract_content_from_file( $handle ),
			'content' => $handle,
	 ...
	 */
	protected static function extract_content_from_url( string $url ): WP_Error|string {
		$url = esc_url_raw( $url );
	 ...
	 * @return string|WP_Error
	 */
	protected static function extract_content_from_file( string $file ): WP_Error|string {
		$filename_length = strlen( self::REQUIRED_FILENAME );
