...
add_action( 'lpcn_gp_project_card', 'LitePress\GlotPress\Customizations\Inc\lpcn_gp_core_card', 10, 3 );
add_action( 'lpcn_gp_project_card', 'LitePress\GlotPress\Customizations\Inc\lpcn_gp_other_card', 10, 3 );
add_action( 'lpcn_gp_project_card', 'LitePress\GlotPress\Customizations\Inc\lpcn_gp_doc_card', 10, 3 );
...
function lpcn_gp_core_card( GP_Project $sub_project, stdClass $translation_set_total, int $sub_sub_projects_count ) {
	if ( 3 !== (int) $sub_project->parent_project_id ) {
		return;
	}
    ...
    <div class="card theme-boxshadow">
        <div class="row g-0">
            <div class="project-top row">
                <div class="col-5 center">
                    <img class="plugin-icon"
                         src="https://img-cdn.haozi.xyz/2021/11/28/f86fbfede99fa9991c24845656e702bb.png"
                         onError="this.src='https://cravatar.cn/avatar/<?php echo md5( $sub_project->slug ) ?>?d=identicon&s=133';">
                </div>
                <div class="col-7">
                    <h5 class="card-title">
                        <a href="<?php echo gp_url_project( $sub_project ) ?>"
                           one-link-mark="yes"><?php echo i18n::get_instance()->translate( 'core_' . (string) $sub_project->slug . '_title', (string) $sub_project->name, (string) $sub_project->path . '/' . (string) $sub_project->slug, true ); ?></a>
                    </h5>
                    <p class="card-text project-description">
                        <small>
							<?php
							echo esc_html( gp_html_excerpt( apply_filters( 'gp_sub_project_description', i18n::get_instance()->translate( 'core_' . (string) $sub_project->slug . '_short_description', (string) $sub_project->description, (string) $sub_project->path . '/' . (string) $sub_project->slug, true ), $sub_project ), 222 ) );
							?>
                        </small>
                    </p>
                </div>
...
function lpcn_gp_doc_card( GP_Project $sub_project, stdClass $translation_set_total, int $sub_sub_projects_count ) {
	if ( 4 !== (int) $sub_project->parent_project_id ) {
		return;
	}
	?>
    <div class="card theme-boxshadow">
