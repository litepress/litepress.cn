	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			if ($this->checkEmpty($apiParams['biz_content'])) {
				...
				throw new Exception("API request failed - encrypted requests are not supported");
			}

			if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

				throw new Exception("encryptType and encryptKey must not be null");
			}

			if ("AES" != $this->encryptType) {

				throw new Exception("encryptType must be AES");
			}

			// 执行加密
			$enCryptContent = AESEncrypt($apiParams['biz_content'], $this->encryptKey, $this->appId);
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		$apiParams = $request->getApiParas();
			...
			// 执行加密
			$enCryptContent = AESEncrypt($apiParams['biz_content'], $this->encryptKey);
			$apiParams['biz_content'] = $enCryptContent;
...
/** 
	...
	private function encryptJSONSignSource($request, $responseContent) {
		...
		$bizContent = AESDecrypt($parsetItem->encryptContent, $this->encryptKey, $this->appId);

		if ( false === $bizContent ) {
			throw new Exception('AESDecrypt error - please verify appId and encryptKey parameters');
		}
