	...
	public function display_vendor_logo_profile() {
		...
		if ( is_tax( WC_PRODUCT_VENDORS_TAXONOMY, $term->term_id ) ) {
			...
			if ( ! empty( $vendor_data['logo'] ) && 'yes' === get_option( 'wcpv_vendor_settings_vendor_display_logo', 'yes' ) ) {
				...
				echo '<section class="woocommerce_archive">
    <div class="d-flex">
        <main class="bg-white theme-boxshadow"><p class="wcpv-vendor-logo">' . wp_get_attachment_image( absint( $vendor_data['logo'] ), 'full' ) . '</p>' . PHP_EOL;
			}
            ...
            if ( ! empty( $vendor_data['profile'] ) && 'yes' === get_option( 'wcpv_vendor_settings_vendor_display_profile', 'yes' ) ) {
                ...
                echo '<div class="wcpv-vendor-profile entry-summary">' . wpautop( wp_kses_post( do_shortcode( $vendor_data['profile'] ) ) )  . PHP_EOL;
            }

            // ratings
            $show_ratings = get_option( 'wcpv_vendor_settings_vendor_review', 'yes' );
			if ( 'yes' === $show_ratings ) {
				echo WC_Product_Vendors_Utils::get_vendor_rating_html( $term->term_id ). '</div>    </main>
    </div>
</section>';
			}
