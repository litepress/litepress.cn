...
class Translations_Service {
	...
	public function update_check( array $projects, array $translations, string $query_type, string $display_type ): array {
		...
		if ( 'core' === $query_type ) {
			...
			$sql     = $wpdb->prepare( "select domain, version, updated from language_packs where type_raw=%s and version=%s and active=1", $query_type, $version );
		} else {
			$sql = $wpdb->prepare( "select domain, version, updated from language_packs where type_raw=%s and domain in ({$domains}) and active=1", $query_type, );
		}
