...
class Permalink_Manager_Core_Functions extends Permalink_Manager_Class {
	...
	function init_hooks() {
		 ...
		 * TODO:WCY
		 *
		 * 因为开了其他设置固定连接的插件，所以这里得把优先级调低，让这个插件最后执行，才好接管特定的链接
		 */
		add_filter( 'request', array($this, 'detect_post'), 9998, 3 );
		...
		*/
		...
		if(filter_var($request_url, FILTER_VALIDATE_URL)) {
			...
			*/
				 ...
				 * TODO:WCY
				 *
				 * 对于产品永远不关闭他的URL重定向，这样就可以在后台去掉所有和url管理相关的功能的同时保留他的url重定向功能，无敌
				 */
				$disabled = true;
